...
int check_left_recursion(rule_t* rules[], size_t rule_count) {
    for (size_t i = 0; i < rule_count; i++) {
        for (size_t j = 0; j < rules[i]->rhs_count; j++) {
            if (rules[i]->rhs[j][0] == rules[i]->lhs[0]) {
                return 1;
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    check_left_recursion(rules, rule_count) ? printf("Left Recursion Detected\n")
                                            : printf("No Left Recursion Detected\n");

    rule_t* new_rules[100];
    size_t new_rule_count = 0;

    for (unsigned i = rule_count; i >= 0; i--) {
        while(1) {
            
        }
    }

    fprintf(stdout, "New Rules:\n");
    for (size_t i = 0; i < new_rule_count; i++) {
        print_rule(new_rules[i]);
    }
        return 0;
